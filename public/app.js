import {loadSavedHistory, saveHistorySnapshot, clearSavedHistory, mergeHistoryPage} from './history-store.js';
...
let lookupQuery=null, storageState='idle';
let lookupState={kind:'idle'},dialogMode='import',inputError=null,toastMessage=null;
const perPage=10;
const sourceLabels=()=>({device:t('Dữ liệu lưu trên thiết bị · chưa xác minh lại'),sample:t("Dữ liệu mẫu giả lập"),json:t("JSON do bạn nhập · chưa xác minh"),manual:t("Dữ liệu bạn nhập · chưa xác minh"),riot:t("Dữ liệu từ Riot API")});
function filters(){const values=Object.fromEntries(new FormData($('#filters')).entries());return {...values,mode:values.queue==='ARAM Mayhem'?'mayhem':'sr'};}
...
function resetFilters(){const mayhem=$('#queue-filter').value==='ARAM Mayhem';$('#filters').reset();if(mayhem)$('#queue-filter').value='ARAM Mayhem';page=1;evidence=null;}
function inferMode(data){return data.matches.every(m=>m.queue==='ARAM Mayhem')?'mayhem':'sr';}
function renderStorage(){
 const messages={idle:'Chưa có bản lưu Mayhem trong phiên này.',saved:'Đã lưu lịch sử Mayhem trên thiết bị này.',restored:'Bản lưu được khôi phục; chưa tra cứu lại Riot.',error:'Không lưu được trên thiết bị. Dữ liệu vẫn còn trong phiên; hãy xuất JSON.',cleared:'Đã xóa bản lưu. Dữ liệu đang xem vẫn còn trong phiên.'};
 $('#storage-status').textContent=t(messages[storageState]);
}
function persistHistory(){
 if(source==='sample'||!history.matches.some(m=>m.queue==='ARAM Mayhem'))return;
 const result=saveHistorySnapshot(history,lookupQuery);storageState=result.ok?'saved':'error';renderStorage();
}
function renderSyncControls(){
 const button=$('#load-older');
 button.hidden=!lookupQuery||!lookupQuery.hasMore||lookupQuery.nextStart>=500||history.matches.length>=MAX_MATCHES||filters().mode!==lookupQuery.mode;
 button.disabled=lookupBusy;button.textContent=t(lookupBusy?'Đang tải…':'Tải thêm trận cũ');
 $('#sync-context').textContent=history.matches.length>=MAX_MATCHES?t('Đã đạt giới hạn 500 trận. Xuất JSON trước khi thay bộ dữ liệu.'):lookupQuery?t(lookupQuery.hasMore?'Tải thêm cho {id} · {mode}':'Hết trang lịch sử hiện có.',{id:lookupQuery.riotId,mode:lookupQuery.mode==='mayhem'?'ARAM Mayhem':'Summoner’s Rift'}):'';
}
function configureMode(){
 const mayhem=filters().mode==='mayhem';$('#role-filter').disabled=mayhem;if(mayhem)$('#role-filter').value='';
 $('#role-stats').closest('section').hidden=mayhem;$('#coaching-tab').disabled=mayhem;
 $('.rule-badge').textContent=t(mayhem?'THỐNG KÊ MAYHEM':'Phân tích theo quy tắc');
 if(mayhem&&view==='coaching')switchView('history');
}
...
function activate(next,nextSource,extra={}){
 rankStates={};backup={history,source,liveDetails,liveMeta,lookupQuery};
 history=next;source=nextSource;liveDetails=next.details||{};liveMeta=extra.meta||null;lookupQuery=extra.query||null;
 $('#filters').reset();$('#queue-filter').value=(lookupQuery?.mode||inferMode(next))==='mayhem'?'ARAM Mayhem':'';page=1;evidence=null;
 refreshChampionOptions();persistHistory();render();
}
...
function profile(){const p=history.profile;$('#profile-identity').innerHTML=`<div class="identity-avatar" aria-hidden="true">${escape(initials(p.riotId))}</div><div><div class="profile-kicker">${t("HỒ SƠ NGƯỜI CHƠI ")}<span class="source-pill">${source==='sample'?t("MẪU GIẢ LẬP"):source==='riot'?'RIOT API':t("DỮ LIỆU NHẬP")}</span></div><h1>${escape(p.riotId)}</h1><p class="profile-meta">${escape(p.region||t("Khu vực chưa nhập"))} <span>·</span>${t(" {value5} trận trong lịch sử ", {value5:history.matches.length})}<span>·</span> ${filters().mode==='mayhem'?t('Sau trận · không áp dụng rank hoặc MMR Mayhem'):p.rank?escape(p.rank)+t(" (tự nhập)"):t("Rank chưa cung cấp")}</p></div>`;
const sourceTitle=source==='device'?t('Bản lưu trên thiết bị.'):source==='sample'?t("Bạn đang xem lịch sử mẫu."):source==='riot'?t("Đã tải dữ liệu từ Riot."):t("Đang phân tích lịch sử bạn cung cấp.");
const sourceMessage=source==='sample'?t('Các trận được giả lập. Tra cứu Riot ID ở trên để xem lịch sử thật.'):source==='riot'?t('Đã tải {count} trận; {skipped} mục bị bỏ qua vì thiếu dữ liệu hoặc sai chế độ.',{count:history.matches.length,skipped:liveMeta?.warnings?.length||0}):t('Dữ liệu nhập hoặc lưu trên thiết bị chưa được xác minh lại với Riot.');
$('#source-banner').innerHTML=`<div><strong>${sourceTitle}</strong> ${escape(sourceMessage)}</div><div class="sample-controls"><label class="sr-only" for="sample-choice">${t("Chọn bộ dữ liệu mẫu")}</label><select id="sample-choice"><option value="mid">${t("Mẫu Mid / ADC")}</option><option value="support">${t("Mẫu Support")}</option><option value="mayhem">${t("Mẫu ARAM Mayhem")}</option></select><button type="button" class="text-btn" id="load-sample">${t("Xem mẫu")}</button>${backup?`<button type="button" class="text-btn" id="undo-data">${t("Hoàn tác thay dữ liệu")}</button>`:''}</div>`;}
function renderSummary(){if(report.coachingAvailable===false){const s=report.stats;$('#summary').innerHTML=[metric(t('TRẬN HỢP LỆ'),s.n,t('Không gồm trận remake')),metric(t('TỈ LỆ THẮNG'),pct(s.winRate),`${t('{value4} thắng',{value4:s.wins})} / ${t('{count} thua',{count:s.losses})}`),metric(t('KDA TỔNG HỢP'),kda(s),`${num(s.kills)} / ${num(s.deaths)} / ${num(s.assists)}`),metric(t('THAM GIA HẠ GỤC'),pct(s.participation),t('{known}/{total} trận có dữ liệu',{known:s.participationKnown,total:s.n})),metric(t('SÁT THƯƠNG / PHÚT'),fmt(s.damage.value),t('{known}/{total} trận có dữ liệu',{known:s.damage.known,total:s.n})),metric(t('VÀNG / PHÚT'),fmt(s.gold.value),t('{known}/{total} trận có dữ liệu',{known:s.gold.known,total:s.n}))].join('');return;}const s=report.stats;const circumference=188.496;$('#summary').innerHTML=`<div class="stat-card winrate-card"><svg viewBox="0 0 76 76" class="win-donut" aria-hidden="true"><circle cx="38" cy="38" r="30" class="donut-track"/><circle cx="38" cy="38" r="30" class="donut-fill" stroke-dasharray="${(s.winRate||0)*circumference} ${circumference}" transform="rotate(-90 38 38)"/></svg><div><span class="metric-label">${t("TỈ LỆ THẮNG")}</span><strong class="metric-value">${pct(s.winRate)}</strong><span class="metric-caption"><span class="win-text">${t("{value4} thắng", {value4:s.wins})}</span> · <span class="loss-text">${t("{count} thua",{count:s.losses})}</span></span></div></div>${metric(t("KDA TỔNG HỢP"),kda(s),t("{value1} / {value2} / {value3} trung bình", {value1:fmt(s.avgKills), value2:fmt(s.avgDeaths), value3:fmt(s.avgAssists)}))}${metric(t("CS / PHÚT"),fmt(s.cs.value),t("{value1}/{value2} trận có CS", {value1:s.cs.known, value2:s.n}))}${metric(t("THAM GIA HẠ GỤC"),pct(s.participation),t("{value1}/{value2} trận tính được KP", {value1:s.participationKnown, value2:s.n}))}${metric(t("CHẾT / 30 PHÚT"),fmt(s.deathsPer30),t("Chuẩn hóa theo thời lượng"))}${metric(t("VISION / PHÚT"),fmt(s.vision.value),t("{value1}/{value2} trận có vision", {value1:s.vision.known, value2:s.n}))}`;}
...
function matchDetail(match){
  return source==='sample'&&match.queue!=='ARAM Mayhem'?sampleMatchDetail(match):liveDetails[match.id]||null;
}
...
function participantRow(p){
  return `<tr class="${p.isPlayer?'player-row':''}"><td><div class="participant-name"><strong>${escape(p.riotId||t("Chưa có Riot ID"))}</strong>${p.isPlayer?`<span class="participant-you">${t("Bạn")}</span>`:''}</div></td><td><div class="participant-champion">${avatar(p.champion,true)}<span><strong>${escape(p.champion)}</strong><br><span class="small muted">${escape(p.role||t("Không áp dụng"))}</span></span></div></td><td><div class="participant-loadout">${renderSpells(p)}${renderItemSlots(p)}</div></td><td>${num(p.kills)} / ${num(p.deaths)} / ${num(p.assists)}</td><td>${num(p.cs)}</td><td>${num(p.damageToChampions)}</td><td>${num(p.goldEarned)}</td><td>${num(p.visionScore)}</td></tr>`;
}
...
function participantTable(id){
  ...
  if(!Array.isArray(detail?.participants)||!detail.participants.length)return `<p class="small muted">${t('Chưa có dữ liệu hai đội cho trận này.')}</p>`;
  const teams=[
  ...
  return `<section class="participants"><div class="participants-heading"><h3>${t("Thống kê hai đội")}</h3><p class="small muted">${escape(detail.sample?t("Mẫu giả lập · "):detail.gameVersion?t("Phiên bản ")+detail.gameVersion+' · ':'')}${detail.sample?t("Không phải dữ liệu người chơi thật"):source==='riot'?t("Số liệu từ Riot API"):t("Dữ liệu nhập hoặc lưu trên thiết bị chưa được xác minh lại với Riot.")}</p></div><p class="team-scroll-hint small muted">${t("Vuốt ngang từng bảng để xem đầy đủ chỉ số →")}</p><div class="participant-teams">${teams.map(participantTeam).join('')}</div></section>`;
}
function row(entry,index){const m=entry.match;if(m.queue==='ARAM Mayhem')return mayhemRow(entry,index);const x=entry.metrics,win=m.result==='Victory',detail=matchDetail(m);const tags=entry.mistakes.map(p=>`<span class="review-tag">${escape(titleFor()[p.id])}</span>`).join('');return `<details class="match-row match-card ${win?'victory':'defeat'}" id="match-row-${index}" data-match-id="${escape(m.id)}"><summary>${renderMatchSummary({match:m,metrics:x,detail,rankState:source==='sample'?{lobbyRank:detail.lobbyRank}:rankStates[m.id],source,dateLabel:dateText(m.playedAt)})}</summary><div class="match-detail">${renderLoadout(matchPlayer(detail))}<div class="detail-metrics"><div><span>${t("Sát thương lên tướng")}</span><strong>${num(m.damageToChampions)}</strong><span>${t("{value7} / phút", {value7:fmt(m.damageToChampions===null?null:m.damageToChampions/m.durationMinutes)})}</span></div><div><span>${t("Vàng kiếm được")}</span><strong>${num(m.goldEarned)}</strong><span>${t("{value9} / phút", {value9:fmt(m.goldEarned===null?null:m.goldEarned/m.durationMinutes)})}</span></div><div><span>${t("Chết / 30 phút")}</span><strong>${fmt(x.deathsPer30)}</strong><span>${t("{value11} lần chết trong trận", {value11:m.deaths})}</span></div><div><span>${t("Vision / phút")}</span><strong>${fmt(x.visionPerMin)}</strong><span>${m.role==='Support'?t("Ngưỡng riêng cho Support"):t("Vai trò ")+escape(m.role)}</span></div></div><div class="match-review"><strong>${t("Tín hiệu của trận này")}</strong><div class="review-tags">${tags||`<span class="small muted">${t("Chưa có chỉ số vượt ngưỡng xem lại.")}</span>`}</div><p class="small muted">${t("Các nhãn là dấu hiệu để xem lại replay. Ưu tiên coaching được xác định trên toàn bộ {value15} trận trong bộ lọc.", {value15:report.stats.n})}</p>${m.notes?`<p class="match-notes"><strong>${t("Ghi chú của bạn · không tự phân tích")}</strong><br>${escape(m.notes)}</p>`:''}${participantTable(m.id)}<span class="match-id">${escape(m.id)}</span></div></div></details>`;}
function mayhemRow(entry,index){
 const m=entry.match,x=entry.metrics,detail=matchDetail(m);
 const status=m.isRemake===true?'Remake · không tính vào thống kê thắng/thua':m.isRemake===null?'Chưa xác minh trạng thái remake':'Thống kê trận · không chấm coaching';
 return `<details class="match-row match-card ${m.result==='Victory'?'victory':'defeat'}" id="match-row-${index}" data-match-id="${escape(m.id)}"><summary>${renderMatchSummary({match:m,metrics:x,detail,source,dateLabel:dateText(m.playedAt)})}</summary><div class="match-detail"><p class="small muted">${t(status)}</p>${renderLoadout(matchPlayer(detail),{mayhem:true})}<div class="detail-metrics">${metric(t('Sát thương lên tướng'),num(m.damageToChampions),t('{value7} / phút',{value7:fmt(m.damageToChampions===null?null:m.damageToChampions/m.durationMinutes)}))}${metric(t('Vàng kiếm được'),num(m.goldEarned),t('{value9} / phút',{value9:fmt(m.goldEarned===null?null:m.goldEarned/m.durationMinutes)}))}${metric('KP',pct(x.participation),`${num(m.kills+m.assists)} / ${num(m.teamKills)}`)}${metric('CS',num(m.cs),t('Chỉ số thiếu được giữ là chưa rõ. Không tự điền bằng 0.'))}</div>${m.notes?`<p class="match-notes"><strong>${t('Ghi chú của bạn · không tự phân tích')}</strong><br>${escape(m.notes)}</p>`:''}${participantTable(m.id)}<span class="match-id">${escape(m.id)}</span></div></details>`;
}
...
$('#match-list').innerHTML=entries.length?entries.slice(start,start+perPage).map((e,i)=>row(e,start+i)).join(''):empty(t("Không có trận trong bộ lọc này."),t("Chọn chế độ phù hợp hoặc nhập thêm lịch sử."));
...
function renderCoaching(){if(report.coachingAvailable===false){$('#coaching-report').innerHTML='';return;}const r=report;if(!r.stats.n){$('#coaching-report').innerHTML=empty(t("Chưa có dữ liệu cho báo cáo"),t("Chọn bộ lọc có trận phù hợp, hoặc nhập lịch sử của bạn."));return;}
...
report={...analyzed,sourceLabel:sourceLabels()[source],limitations:source==='riot'?[...analyzed.limitations.filter(x=>x!==importedLimit),t('Dữ liệu được lấy từ Riot API và có thể được dùng lại từ bộ nhớ tạm. Rank lobby là rank hiện tại, được tải riêng và không dùng để chấm coaching.')]:analyzed.limitations};profile();renderSummary();renderSidebar();renderFocus();renderTrend();renderHistory();renderCoaching();renderStorage();renderSyncControls();if(report.coachingAvailable===false){$('#scope').textContent=t('Đang xem {shown} trận Mayhem · {count} trận tính thống kê · {remakes} remake. Không trộn với Ranked hoặc ARAM thường.',{shown:report.entries.length,count:report.stats.n,remakes:report.remakeCount});return;}$('#scope').textContent=t("Đánh giá {value1}/{value2} trận khớp bộ lọc · {value3} trận ngoài phạm vi được loại · {value4}. Tất cả chỉ số và coaching cùng dùng phạm vi này.", {value1:report.stats.n, value2:report.available, value3:report.excluded, value4:history.ordering==='date'?t("xếp theo ngày giờ"):t("giữ thứ tự nhập, mới nhất trước")});}
function switchView(next,focus=false){if(next==='coaching'&&filters().mode==='mayhem')next='history';view=next;for(const name of ['history','coaching']){const selected=name===next;$(`#${name}-tab`).setAttribute('aria-selected',String(selected));$(`#${name}-tab`).tabIndex=selected?0:-1;$(`#${name}-panel`).hidden=!selected;}if(focus)$(`#${next}-tab`).focus();}
function openDialog(mode){dialogMode=mode;hideErrors();$('#import-panel').hidden=mode!=='import';$('#manual-form').hidden=mode!=='manual';$('#dialog-title').textContent=mode==='import'?t("Nhập lịch sử trận"):t("Thêm trận vào lịch sử");if(mode==='manual'){const form=$('#manual-form');form.reset();form.elements.riotId.value=source==='sample'?'':history.profile.riotId;form.elements.region.value=source==='sample'?'':history.profile.region;if(filters().mode==='mayhem')form.elements.queue.value='ARAM Mayhem';configureManualRole();$('#manual-note').textContent=source==='sample'?t("Trận đầu tiên của bạn sẽ thay lịch sử mẫu; không trộn dữ liệu thật với trận giả lập."):t("Trận này cần thuộc cùng người chơi với lịch sử đang xem. Nếu không có ngày giờ, trận mới được đặt đầu danh sách.");}$('#input-dialog').showModal();}
...
$('#use-template').addEventListener('click',()=>{const sample=exportHistory(sampleHistory(filters().mode==='mayhem'?'mayhem':'mid'));sample.matches=sample.matches.slice(0,3);$('#json-input').value=JSON.stringify(sample,null,2);hideErrors();notify(()=>t("Đã điền 3 trận mẫu giả lập. Thay bằng dữ liệu của bạn."));});
...
$('#manual-form').addEventListener('submit',async e=>{e.preventDefault();if(busy)return;busy=true;const submit=e.submitter||e.target.querySelector('button[type=submit]');submit.disabled=true;hideErrors();try{const data=Object.fromEntries(new FormData(e.target));const previous=source==='sample'?[]:history.matches;if(previous.length&&(data.riotId.trim().toLowerCase()!==history.profile.riotId.toLowerCase()||data.region.trim().toLowerCase()!==history.profile.region.toLowerCase()))throw new InputError([()=>t('Trận này cần thuộc cùng tài khoản và server với lịch sử đang xem.')]);if(previous.length>=MAX_MATCHES)throw new InputError([()=>t("Tối đa 500 trận. Chỉnh JSON để giữ các trận cần phân tích.")]);const date=data.playedAt?new Date(data.playedAt):null;if(date&&!Number.isFinite(date.getTime()))throw new InputError([()=>t("Ngày giờ không hợp lệ.")]);const entry={...data,role:data.queue==='ARAM Mayhem'?null:data.role,id:`manual-${crypto.randomUUID()}`,playedAt:date?date.toISOString():null,isRemake:e.target.elements.isRemake.checked};const next=await historyManualSource.load({profile:{riotId:data.riotId,region:data.region,rank:source==='sample'?'':history.profile.rank},matches:[entry,...previous],details:source==='sample'?{}:history.details});activate(next,'manual');$('#queue-filter').value=entry.queue==='ARAM Mayhem'?'ARAM Mayhem':'';render();$('#input-dialog').close();notify(()=>t('Lịch sử đã cập nhật.'));}catch(error){showError(error);}finally{busy=false;submit.disabled=false;}});
...
if(button.id==='undo-data'&&backup){rankStates={};const previous={history,source,liveDetails,liveMeta,lookupQuery};lookupQuery=backup.lookupQuery||null;history=backup.history;source=backup.source;liveDetails=backup.liveDetails||{};liveMeta=backup.liveMeta||null;backup=previous;$('#filters').reset();$('#queue-filter').value=inferMode(history)==='mayhem'?'ARAM Mayhem':'';page=1;evidence=null;refreshChampionOptions();persistHistory();render();notify(()=>t("Đã khôi phục bộ dữ liệu trước đó."));}
...
function requestErrorMessage(error,fallback){
  if(error?.code==='history_limit')return t('Đã đạt giới hạn 500 trận. Xuất JSON trước khi thay bộ dữ liệu.');
  if(['account_mismatch','mode_mismatch','match_conflict'].includes(error?.code))return t('Không thể gộp lịch sử khác tài khoản hoặc chế độ.');
...
function renderLookupStatus(){
  ...
  button.disabled=lookupBusy;for(const control of $('#riot-lookup').elements)control.disabled=lookupBusy;renderSyncControls();
  ...
  else if(lookupState.kind==='success')status.textContent=t('Đã nhận {loaded} trận trong trang này. Lịch sử đang giữ {total} trận của {id} ({region}).',lookupState.values);
...
function configureManualRole(){const form=$('#manual-form'),mayhem=form.elements.queue.value==='ARAM Mayhem';form.elements.role.disabled=mayhem;form.elements.durationMinutes.max=mayhem?'180':'90';for(const name of ['kills','deaths','assists'])form.elements[name].max=mayhem?'1000':'100';form.elements.teamKills.max=mayhem?'5000':'200';if(mayhem)form.elements.role.value='';else if(!form.elements.role.value)form.elements.role.value='Mid';}
$('#manual-form').elements.queue.addEventListener('change',configureManualRole);
$('#clear-device').addEventListener('click',()=>{const result=clearSavedHistory();storageState=result.ok?'cleared':'error';renderStorage();});
function matchesQuery(data,query){return data.profile.riotId.trim().toLowerCase()===query.riotId.trim().toLowerCase()&&data.profile.region.toLowerCase()===query.region&&data.matches.some(match=>(match.queue==='ARAM Mayhem'?'mayhem':'sr')===query.mode);}
async function syncHistory(older=false){
 if(lookupBusy||older&&!lookupQuery)return;
 const fields=Object.fromEntries(new FormData($('#riot-lookup')));
 const query=older?{...lookupQuery,start:lookupQuery.nextStart,count:Math.min(lookupQuery.count,500-lookupQuery.nextStart)}:{...fields,count:Number(fields.count),start:0};
 if(older&&(!query.hasMore||query.start>=500))return;
 const context=history;
 const saved=!older&&query.mode==='mayhem'?loadSavedHistory():null;
 const base=source!=='sample'&&matchesQuery(history,query)?history:saved&&matchesQuery(saved.history,query)?saved.history:null;
 const baseSource=base===history?source:'device';
 lookupBusy=true;lookupState={kind:'loading'};renderLookupStatus();
 try{
  const result=await riotDataSource.load({riotId:query.riotId,region:query.region,mode:query.mode,count:query.count,start:query.start});
  if(history!==context)throw new InputError([()=>t('Dữ liệu đã thay đổi trong lúc tải. Hãy tra cứu lại.')]);
  if(base===history&&liveMeta?.accountKey&&result.meta.accountKey!==liveMeta.accountKey)throw new InputError([()=>t('Không thể gộp lịch sử khác tài khoản hoặc chế độ.')]);
  const next=result.history?(base?mergeHistoryPage(base,result.history,{prependNew:!older,allowMixed:true}):result.history):history;
  const nextQuery={riotId:next.profile.riotId,region:query.region,mode:query.mode,count:query.count,nextStart:result.meta.nextStart,hasMore:result.meta.hasMore};
  if(!Number.isInteger(nextQuery.nextStart)||nextQuery.nextStart<query.start||nextQuery.nextStart>500||typeof nextQuery.hasMore!=='boolean')throw new InputError([()=>t('Dữ liệu phản hồi không hợp lệ. Vui lòng thử lại.')]);
  const currentFilters=filters();
  activate(next,base&&baseSource!=='riot'?'device':'riot',{meta:result.meta,query:nextQuery});
  if(older){for(const [key,value] of Object.entries(currentFilters)){const element=$('#filters').elements.namedItem(key);if(element)element.value=value;}$('#limit-filter').value='500';}
  else $('#limit-filter').value=String(query.count);
  render();switchView('history');
  lookupState={kind:'success',values:{loaded:result.history?.matches.length||0,total:history.matches.length,id:history.profile.riotId,region:history.profile.region}};
  notify(()=>t('Lịch sử đã cập nhật.'));
 }catch(error){lookupState={kind:'error',error};}
 finally{lookupBusy=false;renderLookupStatus();}
}
$('#riot-lookup').addEventListener('submit',event=>{event.preventDefault();syncHistory();});
$('#load-older').addEventListener('click',()=>syncHistory(true));
const restored=loadSavedHistory();
if(restored){history=restored.history;source='device';liveDetails=history.details;lookupQuery=restored.query;storageState='restored';$('#queue-filter').value=inferMode(history)==='mayhem'?'ARAM Mayhem':'';if(lookupQuery){const form=$('#riot-lookup');for(const key of ['riotId','region','mode','count'])form.elements[key].value=String(lookupQuery[key]);}}
